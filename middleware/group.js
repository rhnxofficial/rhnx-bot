...
Semoga betah ya di sini 😄
      ...
      if (update.action === "promote") {
        await conn.sendMessage(groupId, {
          text: `🔰 Selamat @${username}, sekarang kamu jadi admin grup!`,
          mentions: [member.id]
        });
        console.log(chalk.blue(`[PROMOTE] ${username} dipromote di grup ${groupName}`));
      }

      if (update.action === "demote") {
        await conn.sendMessage(groupId, {
          text: `⚠️ @${username} sudah bukan admin lagi.`,
          mentions: [member.id]
        });
        console.log(chalk.magenta(`[DEMOTE] ${username} dicabut adminnya di ${groupName}`));
      }
    }
  } catch (err) {
