export const teks = {
  toxic: [
    "Bokep",
    "bokep",
    "bct",
    "Bct",
    "Ngentod",
    "Snge",
    "Sange",
    "Blok",
    "ngewe",
    "Ngewe",
    "Ngentd",
    "vcs",
    "Vcs",
    "Coli",
    "coli",
    "colmek",
    "Colmek",
    "toket",
    "Toket",
    "oyo",
    "asu",
    "Ah ah ah"
  ],
  spam: ["Jangan spam dong!", "Bentar dulu kak...", "Ketiknya jangan asal-asalan!"],
  loading: ["_Sedang Di Proses_", "🎌 Tunggu Sebentar Ya Kak"],
  owner: ["Ga boleh ya kak...", "Hanya owner yang bisa melakukan ini", "Kamu bukan owner nih"],
  araara: ["Ara Ara", "Ara ara", "ara"],
  bot: ["Bot", "bot", "Bott", "bott"],
  gasalam: ["p", "P", "Ppp", "pp"],
   salam: ["assalamualaikum", "Assalamualaikum", "Mikum", "mikum"],
  siang: ["Selamat siang", "selamat siang", "koniciwa", "Siang", "siang"],
  pagi: ["Selamat pagi", "selamat pagi", "Ohayou", "Pagi", "pagi"],
  malam: ["Selamat malam", "selamat miang", "Oyasumi", "Malam", "malam"],
  sukses: ["Berhasil kak 👍", "Done kak!", "Siap, sudah dilakukan"],
  private: [
    "Hanya Bisa Digunakan Di Private Chat",
    "Husus Private Aja Loh Kak",
    "Ups! Hanya Bisa Di Private Chat Saja"
  ],
  error: ["Ups error kak 😅", "Gagal nih, coba lagi", "Ada kesalahan, cek command mu"]
};

export const vn = {
  toxic: [
    // dosa pantex
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-o1LD.opus",
    // heeh
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-BJQG.opus",
    // jangan toxic om
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-hJPc.opus"
  ],
  menu: [
    // jj menu
      "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-K1bJ.opus"
  ],
  owner: [
    // ga ada
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-EUPE.opus",
    // gaboleh
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-Viyq.opus",
    // gamau
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-9nKY.opus"
  ],
  araara: [
    // ara ara
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-rGha.opus",
    // ara koplok
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-epzp.opus"
  ],
  bot: [
    // ada apa kak
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-ERJd.opus",
    // iya kak
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-rH8C.opus",
    // kenapa kak
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-wVWq.opus",
    // oyy
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-rbNI.opus",
    // ngomong apaan sih
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-F8xx.opus",
    // lu siapa njir
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-3VbW.opus"
  ],
  salam: [
    // waalaikumsalam
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-m4cQ.opus"
  ],
  spam: [
    // spam kak
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-p95Y.opus"
  ],
  pagi: [
    // selamat pagi
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-Gyt3.opus"
  ],
  siang: [
    // selamat siang
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-TdJP.opus"
  ],
  malam: [
    // selamat malam
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-TloF.opus"
  ],
  kawai: [
    // kawaii desu
    "https://cdn.filestackcontent.com/zHMFviWeT6eDOvu3eJe3"
  ],
  love: [
    // lopyutu
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-ybb5.opus",
    // lopyu kambing
    "https://raw.githubusercontent.com/rhnxofficial/Uploader/main/uploader/rhnx-PJpr.opus"
  ]
};

export const stik = {
  loading: [
    // oke tunggu bentar kak
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737080626461.webp"
  ],
  toxic: [
    // istighfar kak
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737079699099.webp"
  ],
  spam: [
    // bentar dulu kak
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737056134004.webp",
    // jangan spam dong
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737079906743.webp"
  ],
  admin: [
    // hanya qadmin hang bisa kak
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737079414268.webp"
  ],
  tagOwner: [
    // tag owner
    "https://cdn.filestackcontent.com/jbXPAjcHRNSYNalvhbBD"
  ],
  owner: [
    // ga boleh
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737056734997.webp"
  ],
  bug: [
    // bug
    "https://cdn.filestackcontent.com/pPZCyTUJT2WqARnZ4Qo1"
  ],
  del: [
    // apaan tuh kak
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737055956041.webp"
  ],
  cmd: [
    // bodo ah
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737056275723.webp",
    // ga ada dalam menu nih
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737056610209.webp",
    // ga mau aku ngambek
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737079100675.webp",
    // hadeuhh
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737079197059.webp",
    // ketik menu
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737080132952.webp",
    // ketik nya yang benar dong
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737080301188.webp",
    // mau ku jotos
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737080464171.webp"
  ],
  ban: [
    // kakak telah di banned
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737080910200.webp"
  ],
  thanks: [
    // sama sama
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737080768907.webp"
  ],
  gasalam: [
    // ucap salam napa
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737081031478.webp"
  ],
  sukses: [
    // done kak
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737056416633.webp"
  ],
  error: [
    // error kak
    "https://raw.githubusercontent.com/Rangelofficial/Uploade-db/main/uploader/1737056532333.webp"
  ]
};

export const getRandom = (arr) => arr[Math.floor(Math.random() * arr.length)];
