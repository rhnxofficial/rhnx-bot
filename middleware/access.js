...
const RESELLER_DB = "./database/store/reselerpanel.json";

const isReselerPanel = (jid) => {
  if (!fs.existsSync(RESELLER_DB)) return false;

  try {
    const data = JSON.parse(fs.readFileSync(RESELLER_DB));
    return data.some((v) => v.jid === jid && v.active === true);
  } catch {
    return false;
  }
};
...
export default async function checkAccess(m, commandConfig, extras) {
  ...
  if (access.resellerpanel) {
    if (!isReselerPanel(m.sender)) {
      m.reply("⛔ Fitur ini hanya bisa digunakan oleh *Reseller Panel*.");
      return false;
    }
  }

  if (access.storewithprivate) {
    // kalau di grup → wajib group store
    if (m.isGroup && !getGroupStoreIds().includes(m.chat)) {
      m.reply("⛔ Fitur ini hanya bisa digunakan di *Group Store* atau *Private Chat*.");
      return false;
    }
  }
