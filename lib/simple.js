...
export function Func(sock) {
  ...
  conn.sendCarousel = async (jid, cards = [], openingText = "", buttonOpt = {}) => {
    try {
            ...
            buttons: [
              {
                ...
                buttonParamsJson: JSON.stringify({
                  ...
                  url: item.url || "https://api.rhnx.xyz",
                  merchant_url: item.merchant || "https://api.rhnx.xyz"
                })
              }
            ]
          }
  ...
  conn.sendAI = async (jid, text, quotedOrOpts = {}) => {
    try {
      ...
      const opts =
        typeof quotedOrOpts === "object" && quotedOrOpts.key
          ? { quoted: quotedOrOpts }
          : quotedOrOpts;
  ...
  conn.sendMusic = (jid, teks, options) => {
    ...
    conn.sendMessage(jid, { mimetype: "audio/mp4", audio: teks }, { quoted: options });
  };
  ...
  conn.sendThumbnail = (id, teks, m, opts = {}) => {
    ...
    return conn.sendMessage(id, { text: teks, contextInfo }, { quoted });
  };
...
END:VCARD`.trim();
  ...
  try {
    ...
    let media = msg.message[mtype];

    if (mtype === "lottieStickerMessage") {
      media = media?.message?.stickerMessage;
...
export async function smsg(conn, m) {
  ...
  if (!m.isGroup && m.key?.senderPn) {
    const realJid = conn.decodeJid(m.key.senderPn);
...
m.filename =
  ...
  m.message?.stickerMessage?.fileName ||
  null;
    ...
    m.downloadAndSave = async (filename) => {
      try {
        ...
        const trueFileName = path.join(tempDir, filename || `sampah_${makeid(6)}.${type.ext}`);
  ...
  quotedMsg?.imageMessage?.fileName ||
  quotedMsg?.videoMessage?.fileName ||
  quotedMsg?.audioMessage?.fileName ||
  quotedMsg?.stickerMessage?.fileName ||
  null,
        delete: async () =>
  ...
  if (
    [
      ...
      "documentMessage",
      "stickerMessage",
      "lottieStickerMessage" // 🔥 TAMBAH INI
    ].includes(type)
  ) {
