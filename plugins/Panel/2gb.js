  ...
  run: async (m, { conn, args }) => {
    try {
      ...
      await saveToGithub({
        ...
        path: dataPanel.path,
        newData: belipanel,
        message: `add buyer panel 2GB ${username}`
      });

      const text = styleText(`Hai ${username} Selammat Anda Telah Terdaftar Di Panel RHNX
...
Terimakasih Telah Mempercayai *RHNX*`);
      ...
      m.reply("✅ Panel 2GB berhasil dibuat & disimpan ke GitHub");
    } catch (err) {
      console.error(err);
      m.reply("❌ Gagal membuat panel 2GB");
    }
  }
};
