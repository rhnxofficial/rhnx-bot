...
export default {
  name: "unli",
  description: "Buat panel unlimited",
  ...
  run: async (m, { conn, args }) => {
    try {
      ...
      await saveToGithub({
        ...
        path: dataPanel.path,
        newData: belipanel,
        message: `add buyer panel 10GB ${username}`
      });
...
Terimakasih Telah Mempercayai *${panel.name}*`);
      ...
      m.reply(`✅ Panel unlimited dengan nama ${username} Berhasil Di Buat`);
    } catch (err) {
      console.error(err);
      m.reply("❌ Gagal membuat panel unlimited");
    }
  }
};
