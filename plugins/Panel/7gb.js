  ...
  run: async (m, { conn, args }) => {
    try {
      ...
      await saveToGithub({
        ...
        path: dataPanel.path,
        newData: belipanel,
        message: `add buyer panel 7GB ${username}`
      });
...
Terimakasih Telah Mempercayai *${panel.name}*`);
      ...
      m.reply(`✅ Panel 7GB dengan nama ${username} Berhasil Di Buat`);
    } catch (err) {
      console.error(err);
      m.reply("❌ Gagal membuat panel 7GB");
    }
  }
};
